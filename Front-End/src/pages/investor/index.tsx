...
const Investor = () => {
  ...
  const handleDelete = async () => {
    if (open) {
      ...
      const endPoint = `investor/remove`, method = 'DELETE', headers = {
  ...
  React.useEffect(() => {
    const fetchInvestors = async () => {
      ...
      const endPoint = 'investor', id = null, method = 'GET', headers = {
        'Authorization': `Bearer ${token}`
      }, reqData = null;
  ...
  return (
    <>
      ...
      <Box sx={{ width: '80%' }} className='mx-auto py-12 px-10'>
        <h2 className='text-2xl text-gray-600 text-center pb-4 underline'>Investor Profile Data</h2>
                    ...
                    >
                      ...
                      <TableCell align="right" sx={{ textAlign: 'start' }}>
                          ...
                          <img
                            ...
                            title={row.fullName}
                            style={{ width: '200px', objectFit: 'cover', borderRadius: '10px' }}
                          />
                      </TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.phoneNumber}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.companyName}</TableCell>
                      ...
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.skills}</TableCell>
                      <TableCell align="right">{row.location}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.startupStagePreference}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.investmentAmountRange}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.geographicalPreference}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.typeOfInvestment}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.investmentGoals}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.investmentHistory}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.createdAt}</TableCell>
                      <TableCell align="right" sx={{ textAlign: 'start' }}>{row.updatedAt}</TableCell>
                    </TableRow>
                  );
                })}
                {emptyRows > 0 && (
      ...
      {isModalOpen && <UserForm title={"Maintain Investor Profile"} isOpen={isModalOpen} closeHandler={modalCloseHandler} activeUser={activeUser} fields={investorSchema} data={activeCompany} updateId={id} />}
    </>
  );
}
