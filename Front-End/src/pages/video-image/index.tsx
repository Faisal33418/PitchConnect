...
const VideoImage = ({ searchingTxt = null }) => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const endPoint = `video-image/create`, id = null, method = 'POST', headers = {
            ...
            <div>
                <div className="flex gap-2 mb-4">
                    ...
                    <Button
                        ...
                        sx={{ textTransform: 'capitalize', width: '200px', height: 40 }}
                    >
                        Download Recorded
                    </Button>
                </div>
                ...
                {activeRecordings.map(recording => (
                    <div key={recording.id} className="flex justify-center">
                        <video ref={recording.webcamRef} autoPlay width={600} height={600} />
                    </div>
                ))}
            </div>
        );
    ...
    const handleDelete = async () => {
        if (open) {
            const token = localStorage.getItem('token');
            const endPoint = `video-image/remove`, method = 'DELETE', headers = {
                'Authorization': `Bearer ${token}`
            }, reqData = null;
    ...
    return (
        <div className="flex flex-col mx-auto mb-20">
            ...
            <Box sx={{ width: '1000px' }}>
                <h2 className='text-2xl text-gray-600 text-center pt-10 pb-6 underline'>Video & Image Information</h2>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <TableCell align="right" sx={{ textAlign: 'center' }}>
                                                ...
                                                {/* Video Modal */}
                                                <Dialog open={videoModal} onClose={handleCloseVideo} maxWidth="md" fullWidth>
            ...
            {isModalOpen && <UserForm title={"Maintain Banner & Video Data"} isOpen={isModalOpen} closeHandler={modalCloseHandler} activeUser={activeUser} fields={videoImage} data={activeCompany} updateId={id} />}
        </div>
    )
};
