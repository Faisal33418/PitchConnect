...
const SideBar = () => {
    ...
    const [sidebarColor, setSidebarColor] = useState<string | null>(localStorage.getItem('sidebarColor') || '#F6F8F9');
    ...
    return (
        <div>
            ...
            <div className="sticky top-0 h-[calc(100vh-65px)]">
                ...
                {/* Sidebar container */}
                <aside
                    className={`border transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-[240px]' : 'w-0'} overflow-hidden h-[100vh] px-6 border-r rtl:border-r-0 rtl:border-l flex border`}
                    style={{ backgroundColor: sidebarColor }}
                >
                    {/* Side-bar items */}
                    <div className={`${isSidebarOpen ? 'block' : 'hidden'} flex flex-col justify-between items-center mt-6 fixed`}>
                        <nav className="flex-1 -mx-3 space-y-3">

                            {/* Confirming status logics */}
                            {(activeUser?.status === 'pending' || activeUser?.status === 'rejected' || !activeUser?.status) ? <>
                                <h3 className="mt-32 text-red-600 font-semibold">You're approval still pending.<br /> Request to admin for<br />approval.</h3>
                            </> : <>
                                ...
                                <div className="w-[200px] mt-2">
                                    ...
                                    {/* {(isOpen) && (
                                        <> */}
                                    {/* Confirming Investor */}
                                    {(activeUser?.role === 'Investor' || activeUser?.role === 'Admin') && <span className="text-[12px] text-[#84919A] tracking-wide ml-4">Investors</span>}
                                    <div className="mt-2 space-y-1">
                                        {MenuItems.map((item, key) => {
                                            const showItem =
                                                (item.title === 'Find Entrepreneurs' && (activeUser?.role === 'Investor' || activeUser?.role === 'Admin')) ||
                                                ((item.title === 'Entrepreneur Profile' ||
                                                    ...
                                                    (activeUser?.role === 'Entrepreneur' || activeUser?.role === 'Admin'));
                                            if (!showItem) return null;

                                            return (
                                                <>

                                                    {((activeUser?.role === 'Entrepreneur' || activeUser?.role === 'Admin') && item.title === 'Idea Pitches') && <span className="text-[12px] text-[#84919A] tracking-wide ml-4">Entrepreneurs</span>}
                                                    {(activeUser?.role !== 'Admin' && item.title !== 'Idea Pitches') ? <>
                                                    </> : <Link
                                                        key={key}
                                                        className={`group flex items-center px-4 py-2 text-gray-600 rounded transition-colors duration-300 ${activeLink === item.link ? 'bg-[#DFE9E6]' : 'hover:bg-[#DFE9E6]'
                                                            }`}
                                                        href={item.link}
                                                    ...
                                                    {(activeUser?.role === 'Investor' && activeUser?.role !== 'Admin' && item.title === 'Find Entrepreneurs') && <Link
                                                        key={key}
                                                        className={`group flex items-center px-4 py-2 text-gray-600 rounded transition-colors duration-300 ${activeLink === item.link ? 'bg-[#DFE9E6]' : 'hover:bg-[#DFE9E6]'
                                                            }`}
                                                        href={item.link}
                    ...
                    {activeUser?.role === 'Admin' ? (activeUser?.status === 'pending' || activeUser?.status === 'rejected') ? '' : <>

                        {/* Admin side-bar items */}
                        <div className={`${isSidebarOpen ? 'block' : 'hidden'} fixed top-[440px]`}>
                            <hr className="mt-6 mb-6" />
                            <span className="text-[12px] text-[#84919A] tracking-wide">Admin</span>
                            <div className="mt-2">
                                {AdminItems.map((item, key) => (
                                    <Link
                                        className={`group flex px-3 h-8 ml-[-10px] w-[208px] rounded text-gray-600 transition-colors duration-300 ${activeLink === item.link ? 'bg-[#DFE9E6]' : 'hover:bg-[#DFE9E6]'}`}
                                        href={item.link}
                                        onClick={(e) => handlePath(e, item.link)}
                                        key={key}
                                    >
